...
permissoesRouter.put("/usuarios_permissoes_menu/:userId/:menuKey", async (req, res) => {
  ...
  // ON CONFLICT ... DO UPDATE (upsert) é sintaxe compartilhada entre SQLite e Postgres --
  // só precisou trocar `?` por `$n` e colapsar a re-consulta em RETURNING *.
  const { rows } = await query(
    `INSERT INTO usuarios_permissoes_menu (user_id, menu_key, perm_leitura, perm_insercao, perm_edicao, perm_exclusao)
     ...
     RETURNING *`,
    [userId, menuKey, perm_leitura ? 1 : 0, perm_insercao ? 1 : 0, perm_edicao ? 1 : 0, perm_exclusao ? 1 : 0]
  );
