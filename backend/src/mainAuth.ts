...
export function createSession(userId: number): { token: string; expiraEm: string } {
  const token = generateToken();
  const criadoEm = nowIso();
  const expiraEm = new Date(Date.now() + SESSION_TTL_DIAS * 24 * 60 * 60 * 1000).toISOString();
  db.prepare("INSERT INTO usuario_sessoes (sessao_token, user_id, criado_em, expira_em) VALUES (?, ?, ?, ?)").run(
    token,
    userId,
    criadoEm,
    expiraEm
...
export function deleteSession(token: string) {
  db.prepare("DELETE FROM usuario_sessoes WHERE sessao_token = ?").run(token);
}
...
export function getUsuarioDaSessao(token: string): UsuarioLogado | null {
  const row = db
    .prepare(
      `SELECT u.user_id, u.user_nome, u.user_mail, u.user_status, u.user_deve_trocar_senha
       ...
       WHERE s.sessao_token = ? AND s.expira_em > ?`
    )
    .get(token, nowIso()) as
    | (UsuarioLogado & { user_status: string | null })
    | undefined;
...
export function requireUserAuth(req: Request, res: Response, next: NextFunction) {
  ...
  const usuario = token ? getUsuarioDaSessao(token) : null;
  if (!usuario) {
...
export function requireUserOrAdminAuth(req: Request, res: Response, next: NextFunction) {
  const auth = req.header("authorization") ?? "";
  const token = auth.startsWith("Bearer ") ? auth.slice(7) : "";
